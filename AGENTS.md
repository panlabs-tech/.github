...
- **`lefthook.yml`** e **`commitlint.config.mjs`**: o portão 1, local, antes do commit: formatação e verificação da superfície Python, scan de segredos e padrão de mensagem de commit. As ferramentas são equipamento da máquina; este repo versiona a declaração de adesão, que é o que o checker lê.
