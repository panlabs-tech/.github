...
**13 itens aplicados em duas rodadas, por construção.** A segunda rodada existe porque a remoção do diretório que hospedava o runtime vivo fica retida até a migração acontecer: não é conveniência, é o sequenciamento que não pode ser invertido. Replanejar depois deixou **0 itens aplicáveis**.
...
| --- | --- | --- |
| 1ª | 6 links, 2 remoções de fantasma, 1 barra de status, 3 promoções de skill | o diretório do runtime, porque `node, npm, npx, pnpm, pnpx` ainda resolviam lá dentro |
...
| `panlabs-tech/dotfiles` | não existia | existe, com 5 alvos versionados por `chezmoi` |
...
A cláusula de zero redundância é verificada por **varredura do disco**, não suposta. Ela encontra hoje **190 cópias de skill global dentro de 12 repositórios**.
...
## Dois defeitos que a máquina viva expôs

Nenhum dos dois teria aparecido em fixture escrita à mão, e os dois viraram teste.
...
| O bloco de hook no worktree solto | dispara em dobro, e normalizar worktree solto é a [issue #21](https://github.com/panlabs-tech/.github/issues/21) |
