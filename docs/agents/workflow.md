...
- **Portão 1, local:** `lefthook` antes do commit, com formatação, verificação, scan de segredos e padrão de mensagem de commit. Neste repo ele mora em [`lefthook.yml`](../../lefthook.yml), e a anatomia o cobra em quatro itens: que o portão exista, que ele verifique a mensagem, que ele verifique segredos, e que ele rode a ferramenta de cada superfície presente.
